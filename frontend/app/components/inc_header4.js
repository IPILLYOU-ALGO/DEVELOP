...
export default function IncHeader4(props) {
    ...
    return (
        <div className="flex justify-start items-start">
            <div className="flex justify-center items-center flex-grow-0 flex-shrink-0 w-full h-[72px] p-2.5">
                <p className="flex-grow-0 flex-shrink-0 text-xl font-bold text-center text-gray-800">{titles}</p>
                <Link href={urls}>
                <svg
                width="24"
                height="24"
                ...
                <Link href={"/logout"}>
                <p className="text-xl font-bold text-gray-800 right-40">로그아웃</p>
                </Link>
            </div>
        </div>
    )
